...
void ExtraButtonsInterruptEnable() {
		IntEnable(INT_GPIOD);
		HWREG(0x40007408)= 0x00; //diable dual edge interrupts
		HWREG(0x4000740C) = 0xC0; //enable rising edge interrupts
		HWREG(0x40007410) = 0xC0; //mask disable
...
void TimerInterruptSevenSegmentDisplay() {
	...
	PrintNumber(lastKeysPressed[currSevenSeg]);
	GPIO_PORTA_DATA_R ^= 0x0C;
	currSevenSeg ^= 1;
}
